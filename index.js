...
const uri = `mongodb+srv://brianCaceres-Abnb:${process.env.DB_PASS}@cluster0.dhkkn.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0`;
...
async function run() {
  try {
    ...
    const db = client.db("brianCaceres-Abnb"); // Access the correct database
    
    // Get the collections
    const DataOfHotelCollection = db.collection("DataOfHotel");
    const usersDataCollection = db.collection("users");
    const AllHotelListCollection = db.collection("AllHotelList");
